...
func ShopProductPushInfoGet(c *gin.Context) {
	...
	if product == nil {
		c.JSON(http.StatusOK, Result(consts.RESULT_CODE_NOT_FOUND, "not found"))
		return
	}
...
func ShopProductPushInfoSave(c *gin.Context) {
	...
	if pushDto.Pid == "" || (pushDto.Status > 0 && pushDto.NoticePubkey == "") {
		...
		c.JSON(http.StatusOK, Result(consts.RESULT_CODE_ERROR, "Arg error"))
		return
	}
	...
	if product == nil {
		...
		c.JSON(http.StatusOK, Result(consts.RESULT_CODE_ERROR, "Id error"))
		return
	}
	...
	if product.Pubkey != pubkey {
		...
		c.JSON(http.StatusOK, Result(consts.RESULT_CODE_ERROR, "forbidden"))
		return
	}
	...
	if !models.ObjInsert(info, session) {
		c.JSON(http.StatusOK, Result(consts.RESULT_CODE_ERROR, "save fail"))
		return
	}
